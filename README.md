...
# 2. Instalar dependencias de Foundry (se instalan automáticamente)
cd sc
...
El desarrollo principal del proyecto se realizó en **Anvil (Foundry)**, lo que permitió un ciclo de desarrollo rápido y un entorno totalmente controlado.  

Una vez completado el MVP, se intentó realizar el despliegue en la testnet **Sepolia** para validar la interoperabilidad con una red pública EVM.

El código y la configuración del frontend están preparados para hacer **switch entre Anvil, Sepolia u otra red EVM**, pero durante las pruebas en Sepolia se encontraron problemas externos con los endpoints RPC (CORS, respuestas 400, límites de rate y errores de los providers públicos).  

Por esa razón, el entorno final de prueba oficial del proyecto se mantuvo en **Anvil**, asegurando estabilidad para los tests y la presentación.

Aun así, el proyecto **queda listo para apuntar nuevamente a Sepolia** cuando los RPC sean estables.  

Más detalles están disponibles en el archivo relacionado con la testnet:

🔗 **README – Testnet Deployment (Sepolia)**  

`/docs/testnet-sepolia.md`
...
![Infografía del flujo](web/public/flujo-infografia.jpg)