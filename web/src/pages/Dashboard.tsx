      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">
            Tu Rol: {isAdmin ? roleName(5) : user ? roleName(user.role) : 'Cargando...'}
          </h2>
          <p className="text-gray-600">
            {isAdmin && 'Administra usuarios, roles y supervisa toda la cadena de suministro'}
            {!isAdmin && user && user.role === 1 && 'Crea tokens de materias primas y transfiere a fábricas'}
            {!isAdmin && user && user.role === 2 &&
              'Transforma materias primas en productos procesados y transfiere a minoristas'}
            {!isAdmin && user && user.role === 3 && 'Distribuye productos a consumidores'}
            {!isAdmin && user && user.role === 4 && 'Rastrea los productos que recibes'}
          </p>
        </div>
