...
export function CreateToken() {
  const { user, isAdmin, tokenFactory, account } = useWeb3();
  ...
  // Nota: Los admins no pueden crear productos (solo productores y fábricas)
  if (!isAdmin && (!user || !user.approved)) {
    return <Navigate to="/" />;
  }
  
  // Si es admin, mostrar mensaje de que no puede crear productos
  if (isAdmin) {
    return (
        ...
        <Card className="max-w-md">
          ...
          <CardContent>
            <p className="text-gray-600 mb-4">
              Como administrador, no puedes crear productos. Esta función está disponible solo para
